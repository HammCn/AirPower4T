<template>
  <div class="air-table-container" :style="{ height: (autoHeight ? 'auto' : '0px') }">
    ...
    <el-table v-if="allFieldList" :id="tableId" ref="airTableRef" flexible class="air-table"
      :row-key="(row: any) => row.id" :lazy="lazy" :load="load" :default-expand-all="defaultExpandAll"
      :tree-props="treeProps" :data="dataList" height="100%" @select="selectChanged" @select-all="selectChanged"
      @sort-change="sortChanged">
      <el-table-column v-if="showSelect" type="selection" width="40" fixed="left" :reserve-selection="true" />
        ...
        <el-table-column v-if="isSelected(item)" :prop="item.key" :label="item.label" :width="item.width || 'auto'"
          ...
          <template #default="scope">
            ...
            <slot :name="item.key" :data="(scope as any).row" :index="(scope as any).$index">
              <!-- 自动读取枚举 -->
              <div v-if="item.dictionary || getDictionary(entityInstance, item.key)" class="status">
                <!-- 显示状态灯 -->
                <span v-if="item.showColor" class="light" :style="{
                  backgroundColor:
        ...
        <template #header>
          <div class="custom-header">
            ...
            <template v-if="isFieldSelectorEnabled">
              <el-icon v-tip="'配置列字段'" class="air-field-select-icon" @click="isFieldSelectorShow = true">
                <Setting />
              </el-icon>
            </template>
          </div>
        ...
        <template #default="scope">
          <div class="ctrlRow">
            ...
            <template v-if="!hideCtrl">
              ...
              <AButton v-if="!hideDelete" icon-button type="DELETE" :danger="isForceDelete"
      ...
      <template #empty>
        <img src="../assets/img/empty.svg" style="width: 80px;">
        ...
        <div v-if="isFieldSelectorShow" class="air-field-selector-dialog">
          ...
          <div class="air-field-selector-list">
            <el-check-tag v-for="item in allFieldList" :key="item.key" :disabled="item.forceShow"
              :class="item.forceShow ? 'disabled' : ''" :checked="!!selectedFieldList.find(i => i === item.key)"
